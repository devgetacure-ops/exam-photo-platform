...
def transform_point(x: float, y: float, angle_deg: float, scale: float, tx: float, ty: float, cx: float, cy: float) -> tuple[float, float]:
...
def transform_box(box: dict, angle_deg: float, scale: float, tx: float, ty: float, cx: float, cy: float) -> dict:
    ...
    tx_corners = [transform_point(x, y, angle_deg, scale, tx, ty, cx, cy) for x, y in corners]
...
def main():
    parser = argparse.ArgumentParser(description="Generate deterministic variants for visual quality benchmarks.")
    parser.add_argument("--seed", type=int, default=18001, help="Fixed seed for transformations.")
    args = parser.parse_args()

    # Seed the random number generator for reproducibility
    np.random.seed(args.seed)

    repo_root = r"c:\Projects\exam-photo-platform"
    manifest_path = os.path.join(repo_root, "tests", "fixtures", "engine_quality", "fixture_manifest.json")
    annotations_path = os.path.join(repo_root, "tests", "fixtures", "engine_quality", "base_annotations.json")
    base_dir = os.path.join(repo_root, "tests", "fixtures", "engine_quality", "base")
    variants_dir = os.path.join(repo_root, "tests", "fixtures", "engine_quality", "variants")
    ...
    transforms_list = [
        {"name": "clean", "rotation": 0.0, "scale": 1.0, "tx": 0.0, "ty": 0.0, "blur": 0.0, "contrast": 1.0, "noise": 0.0},
        {"name": "rot_l", "rotation": -3.0, "scale": 0.95, "tx": 0.0, "ty": 0.0, "blur": 0.0, "contrast": 1.0, "noise": 0.0},
        {"name": "rot_r", "rotation": 3.0, "scale": 0.95, "tx": 0.0, "ty": 0.0, "blur": 0.0, "contrast": 1.0, "noise": 0.0},
        {"name": "blur", "rotation": 0.0, "scale": 1.0, "tx": 0.0, "ty": 0.0, "blur": 1.5, "contrast": 1.0, "noise": 0.0},
        {"name": "contrast_low", "rotation": 0.0, "scale": 1.0, "tx": 0.0, "ty": 0.0, "blur": 0.0, "contrast": 0.7, "noise": 0.0},
        {"name": "noise", "rotation": 0.0, "scale": 1.0, "tx": 0.0, "ty": 0.0, "blur": 0.0, "contrast": 1.0, "noise": 0.05},
    ]
    ...
    for entry in manifest:
        ...
        for t_cfg in transforms_list:
            ...
            if t_cfg["rotation"] != 0.0 or t_cfg["scale"] != 1.0:
                ...
                v_img = v_img.rotate(t_cfg["rotation"], resample=Image.Resampling.BICUBIC, expand=False)
                
                # Apply scale if not 1.0
                if t_cfg["scale"] != 1.0:
                    sw, sh = int(round(w * t_cfg["scale"])), int(round(h * t_cfg["scale"]))
                    v_img = v_img.resize((sw, sh), Image.Resampling.LANCZOS)
                    # Pad/Crop back to original size centered
                    canvas = Image.new(img.mode, (w, h), (255, 255, 255) if img.mode == "RGB" else 255)
                    ox, oy = (w - sw) // 2, (h - sh) // 2
            ...
            if base_anno["ground_truth_head_box"]:
                t_anno["ground_truth_head_box"] = transform_box(
                    base_anno["ground_truth_head_box"], t_cfg["rotation"], scale, tx, ty, cx, cy
                )

            if base_anno["landmarks"]:
                lm = base_anno["landmarks"]
                lx, ly = transform_point(lm["left_eye"]["x"], lm["left_eye"]["y"], t_cfg["rotation"], scale, tx, ty, cx, cy)
                rx, ry = transform_point(lm["right_eye"]["x"], lm["right_eye"]["y"], t_cfg["rotation"], scale, tx, ty, cx, cy)
                
                custom_lms = {}
                for k, pt in lm["custom_landmarks"].items():
                    px, py = transform_point(pt["x"], pt["y"], t_cfg["rotation"], scale, tx, ty, cx, cy)
    ...
    print(f"Generated {len(variant_manifest)} variants. Manifest saved at {v_manifest_path}")
