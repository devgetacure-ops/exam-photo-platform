...
def run_benchmark():
    ...
    segmenter_model_path = (
        repo_root / "model-assets" / "selfie_multiclass_256x256.tflite"
    )
    ...
    if (
        ...
        detector = MediapipeFaceDetector(
            model_path=face_model_path, expected_sha256=face_sha
        )
    ...
    for entry in entries:
        ...
        try:
            ...
            if detector and segmenter:
                ...
                if len(face_res.detections) == 1:
                    ...
                    head_res = estimator.estimate_head(
                        current_image, face, face.landmarks
                    )
                    with segmenter:
                        seg_res = segmenter.segment_subject(
                            current_image, config=SegmentationConfig()
                        )
                    ...
                    elif entry.get("crop_mode_a_expectation", {}).get(
            ...
            if result.validation.is_valid == expected_valid:
                logger.info(
                    f"  [PASS] Output Preparation Validation matches expectation ({expected_valid})"
                )
                passed += 1
            else:
                logger.error(
