...
def get_direct_url(page_url):
    ...
    try:
        ...
        m = re.search(
            r'href="(https://upload\.wikimedia\.org/wikipedia/commons/[^"]+)" class="internal"',
            html,
        )
        if m:
            return m.group(1)
...
def download_and_normalize(entry: dict, out_dir: str, require_real: bool) -> bool:
    ...
    if entry["source_page_url"] == "local":
        ...
        if not verify_sha256(raw_data, entry["source_sha256"]):
            print(
                f"Error: Local source hash mismatch for {fixture_id}.", file=sys.stderr
            )
            if require_real:
                sys.exit(1)
            return False
    ...
    if not verify_sha256(raw_data, entry["source_sha256"]):
        print(
            f"Error: Downloaded source hash mismatch for {fixture_id}. Expected {entry['source_sha256']}",
            file=sys.stderr,
        )
        if require_real:
            sys.exit(1)
    ...
    try:
        ...
        if not verify_sha256(norm_data, entry["normalized_sha256"]):
            print(
                f"Error: Normalized hash mismatch for {fixture_id}. Expected {entry['normalized_sha256']}",
                file=sys.stderr,
            )
            if require_real:
                sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Download and normalize engine quality fixtures."
    )
    parser.add_argument(
        "--require-real", action="store_true", help="Fail if download fails."
    )
