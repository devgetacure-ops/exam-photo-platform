...
#: Generated sidecar of per-examination facts (DEC-077). Skipped for the same
#: reason as the unavailable list: it lives beside the rule records and is not
#: one, and parsing it as a rule would put it in `unreadable` where it reads
#: as a broken examination. Named outside the `exam_*` namespace because
#: `encode_exam_rules.py` deletes everything matching that prefix.
FACTS_FILENAME = "candidate_facts.json"
...
class Catalogue:
    ...
    #: Per-examination facts, by exam id (DEC-077). Derived from the records
    #: themselves and generated by `scripts/generate_exam_facts.py`.
    facts: dict[str, list[dict[str, object]]] = field(default_factory=dict)
...
def _load_facts(path: Path) -> dict[str, list[dict[str, object]]]:
    """Read the generated facts sidecar, or return nothing.

    A missing or unreadable sidecar is silently empty rather than an error:
    facts are an enhancement to an examination page and their absence must
    never stop a candidate preparing a file.
    """
    if not path.is_file():
        return {}
    try:
...
def load_catalogue(root: Path) -> Catalogue:
    ...
    for path in sorted(root.glob("*.json")):
        if path.name in (UNAVAILABLE_FILENAME, FACTS_FILENAME):
            continue
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
    ...
    return Catalogue(
        ...
        facts=_load_facts(root / FACTS_FILENAME),
    )
