...
export async function processImage({
    ...
    try {
        ...
        throw new Error(
            err.message || "An unknown error occurred during image submission.",
        );
    }
}
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                await errorDetail(
                    response,
                    "Could not load the examination list",
                ),
            );
        }
        return await response.json();
...
export async function getExam(examId: string): Promise<ExamDetail> {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                await errorDetail(response, "Could not load the examination"),
            );
        }
        return await response.json();
    } catch (error) {
...
export async function prepareRequirement({
    ...
    formData.append("enhancement_enabled", String(enhancementEnabled));
    if (progressToken) formData.append("progress_token", progressToken);
    if (challengeToken)
        formData.append("cf_turnstile_response", challengeToken);
    ...
    try {
        ...
        if (response.status === 409) {
            ...
            if (info && info.platform_support)
                throw new RequirementNotServedError(info);
        }
        if (!response.ok) {
...
export async function planDocument({
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                await errorDetail(response, "Could not read the uploads"),
            );
        }
        return await response.json();
    } catch (error) {
...
export async function getKitPackage(kitId: string): Promise<KitPackage> {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                await errorDetail(response, "Could not build the package"),
            );
        }
        return await response.json();
    } catch (error) {
...
export async function deleteJob(jobId: string): Promise<void> {
    ...
    try {
        ...
        const err = error as Error;
        throw new Error(
            err.message || "An error occurred while deleting job resources.",
        );
    }
}
