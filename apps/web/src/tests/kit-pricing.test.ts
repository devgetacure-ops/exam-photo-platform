...
describe("kit price display", () => {
    test("mirrors the server ladder, with eight rupees as the ceiling", () => {
        expect([0, 1, 2, 3, 6].map((n) => tier(n))).toEqual([0, 300, 500, 800, 800]);
    ...
    test("the whole kit counts only files we prepare", () => {
        ...
        expect(price.kitAmount).toBe(800);
