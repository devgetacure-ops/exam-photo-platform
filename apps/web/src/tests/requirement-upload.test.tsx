...
import { RequirementNotServedError, type PrepareRequirementResponse } from "../lib/types";
...
describe("preparing one requirement", () => {
  ...
  test("findings are a distinct state, not filed under success (WEB-002)", async () => {
    prepareRequirement.mockResolvedValue(
      prepared({
        ...
        findings: ["The 50 KB ceiling is our estimate, not a published figure."],
      })
    );
    renderUpload();
    drop(image());
    await waitFor(() =>
      expect(screen.getByText(/ready — with something to check/i)).toBeTruthy()
    );
  ...
  test("a blocked result explains itself and charges nothing", async () => {
    ...
    drop(image());
    await waitFor(() =>
      expect(screen.getByText(/could not prepare this one/i)).toBeTruthy()
    );
  ...
  test("a support-gate refusal is guidance, never an error (DEC-056)", async () => {
    ...
    drop(image());
    await waitFor(() =>
      expect(screen.getByText(/we do not prepare this one/i)).toBeTruthy()
    );
  ...
  test("a transport failure is reported as an error", async () => {
    prepareRequirement.mockRejectedValue(new Error("The processing API is not reachable."));
    renderUpload();
    drop(image());
...
describe("routine normalisation is not a caveat", () => {
  test("a file whose only findings are normalisation notes reads as clean", () => {
    ...
    render(
      <OutcomeResult
        result={prepared({
          outcome: "prepared_with_findings",
          findings: ["INPUT_METADATA_REMOVED", "INPUT_COLOUR_MODE_CONVERTED"],
        })}
        requirementName="Candidate signature"
        onReplace={() => {}}
      />
    ...
    expect(screen.queryByText(/worth checking before you submit/i)).toBeNull();
  });
  ...
  test("a substantive finding still raises the caveat state", () => {
    ...
    expect(screen.getByText(/ready — with something to check/i)).toBeTruthy();
    expect(screen.getByText(/published minimum of 10000 bytes/i)).toBeTruthy();
  });
