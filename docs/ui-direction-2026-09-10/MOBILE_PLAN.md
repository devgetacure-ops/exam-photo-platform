...
| 4 | **Install prompt shipped** (asked once after a useful moment, a card above the bottom bar, Share steps on an iPhone, "Install the app" in the menu). **The owner's first test on a real phone** found every scripted control dead, because the dev server refused its scripts to the LAN address; fixed, with every sideways strip replaced by a layout that fits (DEC-085). The owner then checked the prepare flow on a real phone against a production build: **complete** | `lib/install.ts`, `m/install-card.tsx`, `next.config.ts`, DEC-084, DEC-085 |
