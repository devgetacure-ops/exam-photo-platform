...
| Item | State |
|---|---|
| **Support surface** (C10) | Built under standing authority, 11 September 2026, and reversible. `/support` does in public what a forum is for: every known problem (payment confirming, failed payment, double charge, deleted files, a rejected file, rules that don't match a notice, a missing email) is answered for anyone to read. A candidate's own case goes through a private request with a reference. Nothing a candidate sends is published, so there is no moderation queue and no faces or signatures on a public page. A public discussion board stays the owner's call. **Needs the owner:** a named grievance officer and contact, which Indian e-commerce rules expect on this page, published with the business details. |
