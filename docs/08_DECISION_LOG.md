...
- **Decision**: Add a fourth provenance type, `interim_default`, for a value the platform chose because no source published one. It requires a stated reasoning, is pinned to confidence 1, cannot be approved, and a rule carrying one cannot hold `verified` or `verified_with_ambiguity` status. For the signature gaps specifically, the interim value is the modal published specification, **10-20 KB, JPEG** -- not the arithmetic mean.
